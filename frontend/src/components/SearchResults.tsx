...
import { downloadSearchRunExcel, fetchSearchRunResults } from '../api'
...
export function SearchResults({ runId, run, onProgress }: SearchResultsProps) {
  ...
  async function exportExcel() {
    ...
    try { await downloadSearchRunExcel(runId) }
  ...
  return <section className="results-page" aria-labelledby="results-title">
    ...
    <div className="section-heading"><div><p className="eyebrow">RECHERCHE Nº {runId}</p><h2 id="results-title">Résultats de la recherche</h2><p className="results-summary">{page ? `${page.total} lead${page.total > 1 ? 's' : ''} exploitable${page.total > 1 ? 's' : ''}` : 'Leads exploitables'}{run ? ` sur un objectif de ${run.requested_actionable_leads}` : ''}</p></div><div className="source-actions"><button type="button" className="secondary-button" onClick={() => void exportExcel()} disabled={exporting}>{exporting ? 'Préparation…' : 'Exporter Excel'}</button><button type="button" className="secondary-button" onClick={onProgress}>Voir la progression</button></div></div>
