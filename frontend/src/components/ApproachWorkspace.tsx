...
export function ApproachWorkspace({ lead, onClose }: { lead: CommercialLead; onClose: () => void }) {
  ...
  return <div className="approach-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className={`approach-drawer ${wide ? 'wide' : ''}`} role="dialog" aria-modal="true" aria-labelledby="approach-title" onKeyDown={(event) => {
    ...
    <header className="approach-header"><div><p className="eyebrow">ESPACE COMMERCIAL · PRÉPARATION</p><h2 id="approach-title">{lead.company_name}</h2><p>{pack?.communication_title ?? 'Préparation de l’approche'}{pack?.entry_offer.location && <> · {pack.entry_offer.location}</>}</p></div><div className="approach-header-actions"><button type="button" className="secondary-button" onClick={() => setWide(!wide)}>{wide ? 'Réduire' : 'Agrandir'}</button><button ref={closeButton} type="button" className="modal-close" onClick={onClose} aria-label="Fermer le panneau">×</button></div></header>
